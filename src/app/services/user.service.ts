...
export class UserService {
    ...
    register(token , user_to_register) {
        ...
        return this._http.post(`${this.url}/client/new`, params, {headers: headers})
            .map(res => res.json());

    }
    addAdmin(token , user_to_register) {
        const json 	= JSON.stringify(user_to_register);
        const params = `json=${json}&authorization=${token}`;
        const headers = new Headers({'Content-Type': 'application/x-www-form-urlencoded'});
        console.log(params);
        return this._http.post(`${this.url}/addadmin`, params, {headers: headers})
            .map(res => res.json());
    ...
    getClients(token) {
        ...
        console.log(`${this.url}/client/list`);
        return this._http.post(`${this.url}/client/list`, params, {headers: header})
            .map(res => res.json());
    }
}
